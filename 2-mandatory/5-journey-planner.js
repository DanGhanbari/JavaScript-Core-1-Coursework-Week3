...
*/
function isAccessibleByTransportMode(getTransportModes, strTransport) {
  if (getTransportModes.includes(strTransport)) {
    return true;
  } else {
    return false;
  }
...
/*
  Implement the function getLocationName that
   ...
   - Returns the name of the location
      e.g: "Tower Bridge"
*/
function getLocationName([location, isAccessibleByTransportMode]) {
  if (isAccessibleByTransportMode) {
    return location;
  }
}
...
function journeyPlanner(locations, transportMode) {
  const result = locations
    .filter((ele) => ele.includes(transportMode))
    .map((ele) => ele[0]);
  return result;
